...
for i in range(len_lines):
    clearline = lines[i].split()
    del clearline[1]
    lines[i] = clearline

subs = list(map(''.join, filter(lambda x: len(x) == 1, lines)))
leaders = list(x for x in lines if x[0] not in subs)
for i in leaders:
    if '->' in i:
        i.remove('->')
    len_i = len(i)
...
print("The lowest building is '%s'." % subs[0])
