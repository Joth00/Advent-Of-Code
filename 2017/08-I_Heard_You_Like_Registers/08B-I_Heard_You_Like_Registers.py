...
for i in lines:
    words = i.split()
    if conditions[words[5]](words[4], int(words[6])):
        operators[words[1]](words[0], int(words[2]))
        if register[words[0]] >= highest[1]:
            highest = (words[0], register[words[0]])

largest = max(zip(register.values(), register.keys()))
print("The highest number in the process was '%d'." % highest[1])
print("The name of that register was '%s'." % highest[0])
