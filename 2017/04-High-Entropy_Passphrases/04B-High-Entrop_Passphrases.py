def check_double(my_string):
        ...
        for y in range(len(words)):
            ...
            for z in range(len(words)):
                ...
                if not y == z:
                    if len(i) == len(x):
                        for a in i:
                            validation.append(i.count(a) == x.count(a))
                        if all(validation):
                            valid = False
                            return valid
                        else:
    ...
    valid_phrase = check_letters()
    return valid_phrase


my_input = '''nyot babgr babgr kqtu kqtu kzshonp ylyk psqk
...
valid_phrases = 0
phrases = my_input.splitlines()
for i in phrases:
    if check_double(i):
        valid_phrases += 1

print('There are %d of %d valid passphrases.' % (valid_phrases, len(phrases)))
