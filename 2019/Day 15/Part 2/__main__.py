        ...
        for base_repair_droid in repair_droids:
            ...
            for direction in directions:
                ...
                if status == StatusCodes.HIT_WALL:
                    ...
                    new_repair_droids.append(repair_droid)
                    repair_droid_at_oxygen_system = RepairDroid.from_repair_droid(repair_droid)
        repair_droids = new_repair_droids[:]
        # print_area(area, repair_droids)

    oxygen_system_location = repair_droid_at_oxygen_system.get_location()

    # print_area(area)

    empty_rooms = list(c for c in area.keys() if area[c] == LocationTypes.EMPTY)
    oxygen_rooms = [oxygen_system_location]
    minutes = 0
    while len(empty_rooms) > 0:
        new_oxygen_rooms = oxygen_rooms[:]
        for (x, y) in oxygen_rooms:
            adjacent_locations = [
                (x + 1, y),
                (x - 1, y),
                (x, y + 1),
            ...
            for location in adjacent_locations:
                if location in empty_rooms:
                    area[location] = LocationTypes.OXYGEN
                    empty_rooms.remove(location)
                    new_oxygen_rooms.append(location)
        oxygen_rooms = new_oxygen_rooms[:]
        minutes += 1
        # print_area(area)

    print('Minutes needed to fill all empty rooms with oxygen:', minutes)
    ...
    for y in range(y_max, y_min - 1, -1):
        ...
        for x in range(x_min, x_max + 1):
            if droids is not None and (x, y) in [d.get_location() for d in droids]:
                ...
                line += '\u001b[31m\u2588\u001b[0m'
            elif area[(x, y)] == LocationTypes.OXYGEN:
                line += '\u001b[36m\u2588\u001b[0m'
        print(line)
